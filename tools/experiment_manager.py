...
class ExperimentManager:
    ...
    def __init__(self, experiments_dir='outputs/experiments'):
    ...
    def save_experiment(self, name, description, config, results=None):
        ...
        # Copy model
        if os.path.exists('outputs/models/ddqn_traffic_final.pth'):
            shutil.copy('outputs/models/ddqn_traffic_final.pth', 
                       os.path.join(exp_dir, 'model.pth'))
        
        # Copy checkpoint 400 (often best performing)
        if os.path.exists('outputs/checkpoints/single/ddqn_episode_400.pth'):
            shutil.copy('outputs/checkpoints/single/ddqn_episode_400.pth',
                       os.path.join(exp_dir, 'checkpoint_400.pth'))
        
        # Copy training history
        if os.path.exists('outputs/results/single/training_history.csv'):
            shutil.copy('outputs/results/single/training_history.csv',
                       os.path.join(exp_dir, 'training_history.csv'))
        
        # Copy all checkpoints
        checkpoint_dir = os.path.join(exp_dir, 'outputs/checkpoints/single')
        os.makedirs(checkpoint_dir, exist_ok=True)
        for checkpoint in os.listdir('outputs/checkpoints/single'):
            if checkpoint.endswith('.pth'):
                shutil.copy(os.path.join('outputs/checkpoints/single', checkpoint),
        ...
        self.experiments[exp_id] = {
            ...
            'config': config,
            'outputs/results/single': results,
            'directory': exp_dir
        }
    ...
    def list_experiments(self):
        ...
        for exp_id, exp_data in self.experiments.items():
            ...
            if exp_data.get('outputs/results/single'):
                results = exp_data['outputs/results/single']
    ...
    def get_best_experiment(self, metric='avg_reward'):
        ...
        for exp_id, exp_data in self.experiments.items():
            if not exp_data.get('outputs/results/single'):
                continue
            
            value = exp_data['outputs/results/single'].get(metric)
            if value is None:
                continue
